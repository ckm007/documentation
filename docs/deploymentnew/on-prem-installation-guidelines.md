...
* The deployment process includes the following key components and configurations:
  ...
  * **Nginx Server**: MOSIP uses [Nginx](https://www.nginx.com/) server for:
  ...
  * **Kubernetes (K8s) cluster** :  The deployment involves creating, configuring, and administering a Kubernetes (K8s) cluster:
    ...
    * **Ingress Setup**: Configuring ingress to expose application services outside the K8s cluster.
    * **Storage Class Setup**: Defining the storage class for persistence within the K8s cluster.
    * **Logging System**: Implementing a logging system to continuously scrape logs from all pods as needed.
    * **Monitoring System**: Setting up monitoring to continuously track logs and generate multiple graphs for improved application and cluster management.
    * **Alerting System**: Configuring alerts to notify users about critical events in real time.
...
Below is the Architecture Diagram for V3 MOSIP's reference deployment model.
...
* Create a directory as mosip in your PC and:
    ...
    * Source bashrc so that variables can be used.
...
> Note:  Make sure the permission for `privkey.pem` for ssh is set to 400.
